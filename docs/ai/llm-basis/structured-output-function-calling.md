...
2. **JSON Mode、JSON Schema、Structured Output 的区别**：各自约束什么，不约束什么。
...
## 三层约束：从 JSON Mode 到 Structured Output
...
## Function Calling：模型只生成意图，真正执行在业务侧
...
### Function Calling 和普通 HTTP API 有什么关系
...
### Function Calling 和 MCP Tool 有什么区别
...
### Function Calling 和 Agent Skill 有什么区别
