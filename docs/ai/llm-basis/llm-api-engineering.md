...
> **前置知识**：本文默认你已经理解 Token、上下文窗口、Temperature、Top-p、结构化 Prompt 等基础概念。如果还不熟，建议先看[《万字拆解 LLM 运行机制》](./llm-operation-mechanism.md)和[《大模型提示词工程实践指南》](../agent/prompt-engineering.md)。
